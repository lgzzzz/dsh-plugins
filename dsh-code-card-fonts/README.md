...
本地持久化 Web UI 补丁:把**消息显示区域所有卡片的标题**、**卡片表头的摘要行**、**卡片展开后的内容**、**代码块**、**内联代码**与 **Markdown 表格单元格**统一为 **14px**,并把**消息卡片之间的间距**设为 `calc(14px * 0.5)`(即 **7px**)。内容字号轴 `--dsh-content-font-size`(设置里的「字号大小」)**不再覆盖**:正文、行高等派生变量随设置变化。
...
设置里的「字号大小」由 `dsh-client-ui-theme` 持有(schema 范围 12..17,默认 14),布局服务 `dsh-client-ui-layout` 的 `ThemePresenter` 把它写成 `<body>` 的**内联样式** `--dsh-content-font-size`;正文、代码行高以及 `--dsh-content-font-delta`、`--dsh-content-font-size-secondary` 等派生变量都在 `body` 上从该变量求值。
...
- Markdown 表格的 th/td 由应用侧 `._tableScroll_* th` / `td` 的 `font:` 简写驱动,字号取 `--dsh-content-font-size-secondary`(内容字号 - 1px,比正文小一档);本插件用 `[data-chat-flow-kind] table th/td`(特异性 (0,1,2) > 应用侧 (0,1,1))把字号提升到 14px,不改写行高与字重。
- 卡片根元素设 font-size 无效:卡片内部的大多数文本元素(`.summary`、`.ioText`、`.ioCard` 等)自带显式 font-size,覆盖了继承值。因此按目标元素分别精确覆盖,而不是对根 + 全部后代(`*`)一刀切。
- 标题与摘要均为 14px,通过**更高特异性**的选择器覆盖内容规则实现的,不是调整继承。摘要行(而非摘要 + 全部后代)通过行内结构定位:DisclosureRow 表头里标题恒为第 2 个子元素,其后所有直属子元素即摘要行。
...
| Markdown 表格单元格(表头/正文) | `[data-chat-flow-kind] table th`、`[data-chat-flow-kind] table td` |
...
- 浏览器半部为 TypeScript:`src/client.ts`(入口:注入 `<style>` 卡片字号补丁)+ `src/css.ts`(样式真源);
