...
- **`tools/pre-execute` 钩子**：命中 git 敏感子命令后先解析当前权限，若为完全权限
  则直接 `next()`——deny 与 ask 都不产生，请求原样交给后续监听器 / 链尾；
- **系统提示词区段**：`git-guard:push-policy` 的文本按**当次组装所属会话**动态求值，
  完全权限下返回空串 ⇒ 该区段整体缺席，模型不再收到「须获得用户许可」「不得提交 /
  推送」的要求。
...
- **只认沙箱模式，不认 approval 策略**。GUI 的「完全权限」预设同时写下
  ...
  `ask` 本身已无意义：`approval.request` 在 `never` 下确定性拒绝，ask 会退化成硬拒绝。
...
1. **系统提示词注入（常驻、主动）**：启动时经 `ctx.systemPrompt.section()` 在
   ...
   尝试，也不得以命令替换、别名、脚本包装等间接形式绕过。**区段文本是动态的**：
...
- **权限判定在最内层、且只在将要介入时进行**：钩子先算出决定，只有决定非空
  （即将 deny / ask）才向 `sandboxPolicy` 问一次当前权限；`git status`、`ls`
  这类命令零解析开销。
...
并校验 deny/ask/放行 各分支，含工具名解耦、引号 / 包装词 / 破坏性操作回归，
以及完全权限放行、逐会话生效、只问 sandboxPolicy、失败关闭与提示词区段的动态
求值）：
