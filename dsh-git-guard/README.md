...
插件通过两条路径把「提交／推送都必须经用户许可、用户拒绝后不得重试」的约束
传达给大模型：
...
1. **系统提示词注入（常驻、主动）**：启动时经 `ctx.systemPrompt.section()` 在
   ...
   若用户拒绝了某次提交或推送，不得再次尝试，也不得以命令替换、别名、脚本包装等
   间接形式绕过；如需继续请等待用户指示。这样模型在发起操作之前就能看到该约束。
2. **拦截回传（即时、被动）**：一旦仍检测到 `git push` / `git commit`，ask 的
   `reason` 会作为工具结果原样回传给模型，再次给予同等指示（提醒需用户许可）。
