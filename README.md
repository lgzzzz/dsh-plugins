...
# 全部本地插件（按需删减；写成一行即可）
dsh plugin --profile web remove dsh-code-card-fonts dsh-directory-picker-browse dsh-fork-inbox-guard dsh-fullwidth-chat dsh-git-guard dsh-kbd-hotkeys dsh-new-session dsh-no-right-sidebar
# 重启 App 生效
```
