...
| `dsh-rightbar-diff-split` | `npm run typecheck && npm run build && npm run check`；`node test-diff-split.mjs` | esbuild（平台二进制，无 external）→ `lib/client.js`；`test-diff-split.mjs` 纯 Node、无需浏览器：A 以 Type Stripping 直载 `src/diff-split.ts` + `src/subscriptions.ts`，用类方法形态的 store / slots 桩驱动**持续回正**（判定表全部行、**点击回正的同步性**：用户 toggle 返回后的下一条语句即断言已回正、稳态几十轮非发散通知零写入、`write-failed` 后由下一次通知自愈、加载即对齐、布局 / 右栏 / 审阅三条通知路径各自都能回正、各环缺席或抛错时 no-op、订阅 A–E 建立 / 重建 / 换会话重绑 / 退订），B/C 用 `__ModuleLoader__` 桩载入产物校验包名 / `inject` / `apply` 装配（含加载即对齐与产物级「模拟点击页头按钮 → 同步回正」）与容错 |
