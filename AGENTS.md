...
| `dsh-git-guard` | `npm run typecheck`；`node test.mjs` | `test.mjs` 以 Type Stripping 运行时验证 deny / ask / 放行各分支（含工具名解耦、引号与包装词、破坏性操作回归），以及完全权限放行、权限逐会话生效、只取用 `sandboxPolicy`、服务缺席 / 无 `resolve` / 抛错的失败关闭、提示词区段的动态求值 |
