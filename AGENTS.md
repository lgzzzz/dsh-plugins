...
| `dsh-rightbar-diff-split` | Client only（TS；宿主占位） | 右栏「变更审阅」diff 的左右对比与右栏**全屏**恒等（**持续回正**）：订阅布局 store 的 `layoutInfo.rightbarFullscreen`、会话级右栏 store 与变更审阅视图 store（另加 `uiSession.current` 换会话、`slots.subscribe` 三键的重绑），任一次通知 / 注册变化 / 换会话以及加载各就地跑一次「读全屏 + 读当前会话当前面板当前标签 → `split ≠ 全屏值` 时先读后 toggle」；因 `toggledSplit` 是 toggle 且该 store 是 sync flush，回正在触发它的同一次同步通知栈内完成 ⇒ 页头「左右对比」按钮降级为**只读状态指示器**（视觉上点了没反应）。**不做事件筛选、不记上次写入值、不轮询、不排定时器、不回退 DOM**；另一独立职责 = **悬停浮窗抑制**：向 `document.head` 追加一条 `[data-changes-hover-preview] { display: none }`（聊天区「改动卡片」文件行 `HoverCard` 浮窗的稳定标记，上游 scope 内唯一），摘除挂 `ctx.effect`，与服务面零耦合（`ctx` 全缺照装、无 DOM 则 no-op） |
...
| `dsh-rightbar-diff-split` | `npm run typecheck && npm run build && npm run check`；`node test-diff-split.mjs` | esbuild（平台二进制，无 external）→ `lib/client.js`；`test-diff-split.mjs` 纯 Node、无需浏览器：A 以 Type Stripping 直载 `src/diff-split.ts` + `src/subscriptions.ts`，用类方法形态的 store / slots 桩驱动**持续回正**（判定表全部行、**点击回正的同步性**：用户 toggle 返回后的下一条语句即断言已回正、稳态几十轮非发散通知零写入、`write-failed` 后由下一次通知自愈、加载即对齐、布局 / 右栏 / 审阅三条通知路径各自都能回正、各环缺席或抛错时 no-op、订阅 A–E 建立 / 重建 / 换会话重绑 / 退订），B/C 用 `__ModuleLoader__` 桩载入产物校验包名 / `inject` / `apply` 装配（含加载即对齐与产物级「模拟点击页头按钮 → 同步回正」）与容错（**样式补丁 `src/hover-preview.ts` 无自动化覆盖**：脚本是纯 Node 无 `document`，靠该插件 README 的人工清单核对） |
...
③**持续订阅并不等于「持续纠正」**：幂等读（「现读快照的值是否已等于现读的期望值」）本身就
让稳态通知退化为 no-op，故**不要**用「与上一次快照比对」筛事件、也**不要**记「上次写入值」
——两者都会让「绕过插件造成的发散」被漏掉。若写面是 toggle 且目标 store 是 sync flush
（如该插件的 `toggledSplit`），回正必须**就地**在触发它的同一次同步通知栈内完成，
否则会出现一帧中间态。
